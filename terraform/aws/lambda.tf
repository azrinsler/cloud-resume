...
resource "aws_lambda_function" "delete_recipe_lambda_function" {
  ...
  source_code_hash = filebase64sha256(local.delete_recipe_lambda_path)
...
resource "aws_lambda_event_source_mapping" "delete_recipe_lambda_sqs_event_source_mapping" {
  event_source_arn = aws_sqs_queue.delete_recipe_lambda_queue.arn
  function_name    = aws_lambda_function.delete_recipe_lambda_function.arn
}

# RECIPE API LAMBDA - PUBLIC
# the actual runtime for this function is Kotlin, but it compiles to Java and that's what AWS cares about
resource "aws_lambda_function" "recipe_api_lambda_public_function" {
  function_name = var.recipe_api_lambda_public_class
  s3_bucket = aws_s3_bucket.packaged_source_bucket.id
  s3_key    = aws_s3_object.recipe_api_lambda_source.key
  # Supported Runtimes: https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html#runtimes-supported
  runtime = "java21"
  handler = "azrinsler.aws.${var.recipe_api_lambda_public_class}::handleRequest"
...
resource "aws_lambda_permission" "gateway_recipe_api_lambda_public_permission" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.recipe_api_lambda_public_function.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn = "${aws_apigatewayv2_api.primary_gateway.execution_arn}/*/*"
}

# RECIPE API LAMBDA - USER
# the actual runtime for this function is Kotlin, but it compiles to Java and that's what AWS cares about
resource "aws_lambda_function" "recipe_api_lambda_user_function" {
  function_name = var.recipe_api_lambda_user_class
  s3_bucket = aws_s3_bucket.packaged_source_bucket.id
  s3_key    = aws_s3_object.recipe_api_lambda_user_source.key
  # Supported Runtimes: https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html#runtimes-supported
  runtime = "java21"
  handler = "azrinsler.aws.${var.recipe_api_lambda_user_class}::handleRequest"
  source_code_hash = filebase64sha256(local.recipe_api_lambda_user_path)
  role = aws_iam_role.lambda_exec.arn
  timeout = 30 // specified in seconds
  // memory defaults to 128 which is too small for a Kotlin function using heavy dependencies like aws sdk and jackson
  // (by setting this to a higher value we can VASTLY improve our cold start times, in particular)
...
# gives permission for api gateway to invoke the recipe api lambda
resource "aws_lambda_permission" "gateway_recipe_api_lambda_user_permission" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.recipe_api_lambda_user_function.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn = "${aws_apigatewayv2_api.primary_gateway.execution_arn}/*/*"
}
